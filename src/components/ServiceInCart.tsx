...
export default function ServiceInCart({ service, quantity }: Props) {
  const { i18n, t } = useTranslation();
  const { increaseServiceQuantity, decreaseServiceQuantity, removeService } =
    ...
    <div className='flex flex-col md:flex-row w-full bg-foreground rounded-xl border border-foreground shadow-2xl h-fit gap-1 md:gap-4 xl:gap-6 p-1'>
      <div className='flex flex-row gap-2 md:gap-4 xl:gap-6 h-52'>
        <div
          ...
          onClick={() => removeService({ documentId: service.documentId })}
        >
        ...
        <li className='flex flex-row justify-between'>
          ...
          <div className='flex'>
            <button
              disabled={quantity === 1}
              onClick={() =>
                decreaseServiceQuantity({ documentId: service.documentId })
              }
            >
            ...
            <button
              onClick={() =>
                increaseServiceQuantity({ documentId: service.documentId })
              }
            >
